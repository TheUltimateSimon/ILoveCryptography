...
public class DES {
	...
	public void setBlockFromBase64(String base64Input) {
        byte[] decodedBytes = Base64.getDecoder().decode(base64Input);
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                block[i * 8 + j] = (decodedBytes[i] & (1 << (7 - j))) != 0 ? 1 : 0;
            }
        }
    }
	public String getBlockAsBase64() {
        byte[] encodedBytes = new byte[8];

        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                encodedBytes[i] |= block[i * 8 + j] << (7 - j);
            }
        }
	...
	public static void main(String[] args) throws Exception {
		...
		Scanner fin = new Scanner(new File("ciphertext-2023.txt"));
		FileWriter fout = new FileWriter(new File("test2.txt"));
		FileWriter fout2 = new FileWriter(new File("test3.txt"));
		
		// Set up the keys.
		int[] mykey= new int[64];//= getKey(key);
		for(int x = 0; x < 64; x++)
		{
			mykey[x] = (int) (Math.random() * 2);
		}
		for(int x = 0; x < 64; x++)
		{
			System.out.print(mykey[x]);
		}
		System.out.println();
		...
		while (fin.hasNext()) {
			...
			code.setBlockFromBase64(myblock);
			code.encrypt();
			code.print(fout);
			code.decrypt();
			code.print(fout2);			
