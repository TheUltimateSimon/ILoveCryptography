...
public class DES1 {
	...
	// solution.
	public DES1(int[] thekey) throws Exception {
	...
	public String returnBinary(String radix11) {
		StringBuilder binaryStringBuilder = new StringBuilder();
        for (int i = 0; i < radix11.length(); i++) {
            char radixChar = radix11.charAt(i);
            int radix = Character.getNumericValue(radixChar);

            // Convert the Radix-64 value to 6-bit binary
            String binaryValue = String.format("%6s", Integer.toBinaryString(radix)).replace(' ', '0');

            binaryStringBuilder.append(binaryValue);
        }

        // Add four zeros at the end to make it 64 bits
	...
	public void roundDE(int num) {
		...
		int[] xorans = XOR(expanded, roundkeys[15 - num]);
	...
	public static void main(String[] args) throws Exception {
		...
		String infilename = "ciphertext-2023.txt";
		...
		//System.out.println("Enter the output file 2.");

		// For Decryption File
		String outfilename2 = "test2.txt";
		
		//System.out.println("Enter the key in hex.")
		// Change this to key we want to iterate through.
		String key = "1A2B3C4D5E6F7A8B";
		// Set up the keys.
		int[] mykey = getKey(key);
		DES1 code = new DES1(mykey);
		code.setKeys();
		
		Scanner fin = new Scanner(new File(infilename));
		FileWriter fout = new FileWriter(new File(outfilename));
		FileWriter fout2 = new FileWriter(new File(outfilename2));
		
		
		// Read in each block and process...
		while (fin.hasNext()) {
			String radix = fin.next();
			String myblock = code.returnBinary(radix);
